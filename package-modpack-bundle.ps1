<#
.SYNOPSIS
    Package the one-time modpack tester bundle: the non-committed build
    artifacts a tester needs so `deploy-modpack.ps1` and `deploy-modpack-cp.ps1`
    run with no clone and no build on their machine.

.DESCRIPTION
    Existing power-users already pull source and run deploy.ps1 to test changes
    before a public release. To let them also run the two modpack deploys (and
    flip freely between vanilla, the VP modpack, and the CP-only modpack), they
    need a handful of things that are not in git and are normally clone- or
    build-derived:

      build/modpack-out      the baked VP modpack DLC (CP + VP + fork DLL + DB)
      build/vendor/vp        the staged VP vendor overlay the DLC payload pulls in
      build/vp-runtime       the VP-completion assets deploy-modpack would
                             otherwise take from the Community-Patch-DLL clone
                             (VPUI, the VP Expansion2.Civ5Pkg, minor-civ sounds,
                             loading tips)
      build/modpack-cp-out   the baked Community-Patch-only modpack DLC (CP mod +
                             fork DLL + CP-only merged DB)
      build/vendor/cp        the staged CP vendor overlay the CP DLC payload pulls
                             in

    The CP modpack needs no clone-derived runtime assets: it embeds Community
    Patch in full and uses the stock BNW base, so deploy-modpack-cp performs no
    plain-VP completion. That is why there is no cp counterpart to vp-runtime.
    ...
    This script stages build/vp-runtime from the clone, then zips all of the
    above into one archive whose internal layout mirrors the repo's build/
    subtree. The tester extracts it into the repo's build/ directory once;
    afterwards `git pull` + one of the deploy scripts is their whole loop.

    Prerequisites (maintainer, one-time per VP pin):
      build-modpack.ps1                                   -> build/modpack-out
      build-modpack-cp.ps1                                -> build/modpack-cp-out
      py tools/vendoring/vendor.py generate --engine vp   -> build/vendor/vp
      py tools/vendoring/vendor.py generate --engine cp   -> build/vendor/cp
...
$modpackCpBuildDir = Join-Path $repoRoot 'build\modpack-cp-out'
...
$zipPath      = Join-Path $OutDir "civ-v-access-modpack-bundle-$modVersion.zip"

# ---- 1. Verify the baked inputs exist ----
if (-not (Test-Path (Join-Path $modpackBuildDir 'MPModsPack.Civ5Pkg'))) {
    throw "Baked VP modpack missing at $modpackBuildDir. Run build-modpack.ps1 first."
}
if (-not (Test-Path (Join-Path $modpackCpBuildDir 'MPModsPack.Civ5Pkg'))) {
    throw "Baked CP modpack missing at $modpackCpBuildDir. Run build-modpack-cp.ps1 first."
}
...
if (-not (Test-Path (Join-Path $vendorCpDir 'UI'))) {
    throw "CP vendor stage missing at $vendorCpDir. Run: py tools/vendoring/vendor.py generate --engine cp --mods `"$ClonePath`""
}
...
# ---- 3. Stage all the trees under one build/ root, then zip once ----
...
Copy-Item -LiteralPath $modpackCpBuildDir -Destination (Join-Path $stageBuild 'modpack-cp-out') -Recurse -Force
...
Copy-Item -LiteralPath $vendorCpDir    -Destination (Join-Path $stageBuild 'vendor\cp')   -Recurse -Force
...
Write-Host ""
Write-Host "Modpack tester bundle written:"
...
Write-Host "     build/modpack-cp-out, build/vendor/vp, build/vendor/cp,"
Write-Host "     build/vp-runtime; existing build/ contents are kept)."
Write-Host "  2. From then on: git pull, then ./deploy-modpack.ps1 (VP modpack),"
Write-Host "     ./deploy-modpack-cp.ps1 (CP-only modpack), or ./deploy.ps1"
Write-Host "     (vanilla). No clone, no build needed."
