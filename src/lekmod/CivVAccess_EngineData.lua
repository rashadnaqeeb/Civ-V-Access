...
-- Patch. Crucially for this seam, it does not diverge on the reads the port
-- exists to mediate, so every body here is the vanilla call -- identical to
-- src/dlc/UI/InGame/CivVAccess_EngineData.lua. The findings that justify
-- that (do not re-derive; verified against the LekMod DLL source):
--   * Happiness is unchanged vanilla signed surplus. GetExcessHappiness /
--     GetHappiness / GetUnhappiness are byte-identical to the SDK, so
--     happinessSummary stays mode "surplus" with no approval branch (this is
--     the one place VP diverges hardest, and LekMod simply does not).
...
--     drift on LekMod, but the call sites still route through here so the mod
--     stays engine-portable).
--   * Extension bindings -- Lua methods the stock engine does not expose and
--     our C++ fork adds (mission queue, pathfinder, line of sight, build-route
--     finder). We ported the SAME additions onto LekMod's DLL under their
--     vanilla names, so these bodies are the vanilla calls, forkPresent-gated
--     so a LekMod session without our fork DLL degrades to a safe value and
--     logs rather than throwing a method-not-found the engine swallows.
--
-- Detection. LekMod has no Game.IsCustomModOption (VP's balance-mode signal),
-- so LekMod-specific content is detected by the presence of a LekMod-only
-- binding -- Game.GetProposalStatus for the voting system. forkPresent() gates
-- OUR ported bindings; a LekMod session without our fork degrades to
-- vanilla-correct, so that probe gates additions, not correctness. LekMod's
-- own additive surfaces (the proposal-voting system, deal-review prompt, the
-- extra combat-preview modifiers) attach here as they land, added across the
-- vanilla / VP / LekMod bodies together so the set-parity test stays green.
--
...
-- Is our engine fork DLL deployed? The fork registers several Game-level
-- bindings; if those resolve, the Unit / Plot fork bindings resolve too, so
-- the Game methods are the canary. A stock-LekMod install (LekMod's own DLL,
-- never deployed against, or a sighted-MP partner with only the GUID match)
-- makes every extension binding degrade rather than throw. This is the
-- capability source the Boot probe logs from and the binding wrappers gate on.
