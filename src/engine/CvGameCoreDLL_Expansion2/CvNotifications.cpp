...
bool CvNotifications::IsNotificationExpired(int iIndex)
{
	...
	case NOTIFICATION_PRODUCTION:
	{
		...
		// CIVVACCESS: the city still sits on this plot but now belongs to
		// someone else (traded or captured away this turn). The plot-scoped
		// lookup above lost the ownership check the old getCity(id) path had,
		// so a production notification for a city you no longer own never
		// expired. Expire it so it stops blocking end-turn and clears from
		// the notification list on the next Update().
