...
local function restorePickerCursor(handler, state, pickerTabIdx)
    ...
    if tab == nil or tab._items == nil then
        return
    end
    local found = false
    local foundPath
...
local function entryIsActivatable(self)
...
function PickerReader.create()
    ...
    local function activateEntry(entry, menu, playSound)
        ...
        if type(readerItems) ~= "table" or #readerItems == 0 then
            SpeechPipeline.speakInterrupt(Text.key("TXT_KEY_CIVVACCESS_PICKER_READER_EMPTY"))
            return false
        end
    ...
    local function labelForId(id)
        ...
        if handler == nil or handler.tabs == nil then
            return ""
        end
    ...
    function session.Entry(spec)
        ...
        elseif spec.visibilityControlName ~= nil then
            ...
            if item._visibilityControl == nil then
                Log.warn("PickerReader.Entry: missing visibility control '" .. spec.visibilityControlName .. "'")
            end
        end
    ...
    function session.install(ContextPtr, config)
        ...
        local emptyReaderText = config.emptyReaderText or Text.key("TXT_KEY_CIVVACCESS_PICKER_READER_NO_SELECTION")
        ...
        local bmSpec = {
            ...
            tabs = {
                ...
                },
                {
                    ...
                    nameFn = function()
                        return labelForId(state.selectedId)
                    end,
