...
HideWindow = function()
    if not _clicked and g_PopupInfo ~= nil and g_PopupInfo.Type == ButtonPopupTypes.BUTTONPOPUP_DECLAREWARMOVE then
...
local function invokeSlot(idx)
    ...
    if g_PopupInfo ~= nil and g_PopupInfo.Type == ButtonPopupTypes.BUTTONPOPUP_DECLAREWARMOVE then
        local UC = civvaccess_shared.modules and civvaccess_shared.modules.UnitControl
        if UC ~= nil then
    ...
    if fn ~= nil then
        ...
        if not ok then
            Log.error("DeclareWarPopupAccess: button " .. idx .. " callback failed: " .. tostring(err))
        end
    end
