-- Demographics row builder, shared between the standalone Demographics
-- popup (F9 in-game) and the Demographics tab embedded in EndGameMenu's
-- end-of-game screen. Both Contexts surface the same per-metric speech
-- (Population, Crop Yield, GNP, etc., each speaking the active player's
-- value, rank, and the rival best / average / worst figures) -- the only
-- difference is which Context's ContextPtr the items get installed under,
-- so the row builder lives here and the install wrappers (DemographicsAccess
-- for F9, EndGameMenuAccess for the end-game tab) call into it.
--
-- Per-metric formulas mirror Demographics.lua's GetXValue functions
-- verbatim so the spoken numbers match the column a sighted onlooker sees.
-- Pure functions, no captured state, no install side effects -- safe to
-- include() in any Context that already loads PopupBoot.
...
-- Per-metric value functions exported for test seams / future extension.
DemographicsRows.valuePopulation = valuePopulation
DemographicsRows.valueFood = valueFood
DemographicsRows.valueProduction = valueProduction
DemographicsRows.valueGold = valueGold
DemographicsRows.valueLand = valueLand
DemographicsRows.valueArmy = valueArmy
DemographicsRows.valueApproval = valueApproval
DemographicsRows.valueLiteracy = valueLiteracy
DemographicsRows.bestOf = bestOf
DemographicsRows.worstOf = worstOf
DemographicsRows.averageOf = averageOf
DemographicsRows.rankOf = rankOf
