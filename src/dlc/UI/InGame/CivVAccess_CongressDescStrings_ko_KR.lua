-- Mod-authored World Congress splash painting descriptions, ko_KR overlay.
-- Baseline in CivVAccess_CongressDescStrings_en_US.lua.
CivVAccess_Strings = CivVAccess_Strings or {}

CivVAccess_Strings["TXT_KEY_CIVVACCESS_CONGRESSDESC_MISSING"] = "이 이미지에 대한 설명이 없습니다."

CivVAccess_Strings["TXT_KEY_CIVVACCESS_CONGRESSDESC_LEAGUE_SPECIAL_SESSION_START_WORLD_CONGRESS"] =
    "회의장 뒤편에서 청중의 머리 너머로 바라본 세계 회의의 개회 장면입니다. 앞쪽에는 카펫을 깐 단상 위에 붉은 천을 드리운 긴 탁자를 둘러싸고 대표단이 앉아 있으며, 각 자리마다 흰 서류 한 장이 놓여 있습니다. 흰 로브와 흰 두건을 두른 한 인물이 탁자 상석에 서서 회의장에 연설하고, 그의 옆에는 검은 수염의 남자가 서 있습니다. 청중은 나무 의자가 늘어선 열에 앉아 그들을 마주 보며, 터번과 소모자와 맨머리가 한데 모여 있습니다. 오른쪽 벽을 따라 늘어선 창백한 기둥들 사이에서 더 많은 방청객이 지켜보고 있습니다."

CivVAccess_Strings["TXT_KEY_CIVVACCESS_CONGRESSDESC_LEAGUE_PROJECT_WORLD_FAIR"] =
    "거대한 대관람차가 오른쪽 위 화면 밖으로 솟아오르며, 가장자리에는 밀폐된 승객용 칸이 매달려 있습니다. 대관람차의 받침대는 리벳 박힌 초록색 큰 아치 뒤로 내려와 있으며, 그 아치는 FERRIS WHEEL이라 적힌 창백한 벽 위를 가로지르고 있습니다. 박람회 관람객들이 아래의 넓은 먼지 길을 가로질러 걸어가는데, 긴 드레스를 입은 여인들과 외투에 중산모 및 실크해트를 쓴 남자들이 보입니다. 왼쪽으로는 채색된 간판이 달린 낮은 박람회 건물들이 줄지어 늘어서 있으며, 그 끝에서 흰 돔 전당이 연한 파란 하늘 아래 멀리 솟아 있습니다."

CivVAccess_Strings["TXT_KEY_CIVVACCESS_CONGRESSDESC_LEAGUE_PROJECT_WORLD_GAMES"] =
    "경기 출발의 첫 순간을 담은 장면입니다. 선수 넷이 머리를 숙인 채 어두운 트랙을 가로지르는 전경에서 출발선을 박차고 앞으로 힘차게 치고 나갑니다. 그 뒤 왼쪽에는 흰 긴 코트를 입은 출발 심판이 서서 권총을 높이 들고 있으며, 아직 연기가 피어오르고 있습니다. 뒤편 전체는 빼곡히 들어찬 관중석으로, 군중은 색의 점들로 찍혀 있습니다. 오른쪽 트랙 옆으로는 흰 사다리 관람대와 높은 기둥들이 늘어서 있고, 그 너머로는 인필드의 초록이 펼쳐져 있습니다."
