...
-- Pixel-space bearing of the (fromX, fromY) -> (toX, toY) endpoint delta,
-- in radians CCW from east, normalized to [0, 2pi). Returns nil at zero
-- delta. Pointy-top column-step is sqrt(3) units wide, row-step is 1.5
...
-- (toX, toY). Returns (dcol, drow) -- the raw hex delta with the half-
-- row parity offset applied to dcol so NE / NW / N don't collapse onto
-- the same column delta. Handles map wrap so a target one west on a
-- wrapped map reads (-1, 0) rather than (W-1, 0). Endpoints return
-- (0, 0). No normalization and no screen-space pixel scaling: callers
-- decide what to do with the raw deltas. Beacons.updateBeaconParams
-- maps dcol to pan and drow to pitch semitones, scaling each axis
-- independently rather than collapsing both to a single bearing angle.
function HexGeom.displacement(fromX, fromY, toX, toY)
    toX, toY = nearestWrappedTo(fromX, fromY, toX, toY)
