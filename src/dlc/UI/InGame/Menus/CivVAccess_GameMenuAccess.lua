...
local function showMainContainer()
    showPanel(Controls.MainContainer)
end
local function showDetailsPanel()
    showPanel(Controls.DetailsPanel)
end
local function showModsPanel()
    showPanel(Controls.ModsPanel)
end
...
local function pushExitConfirmSub()
    local subName = "GameMenuExitConfirm"
        ...
        items = {
            ...
            BaseMenuItems.Choice({
                ...
                activate = function()
                    ...
                    -- parent GameMenu handler's focused item mid-teardown
                    -- is garbage speech.
...
local function buildActionsItems()
    return {
        BaseMenuItems.Button({
            controlName = "ReturnButton",
            textKey = "TXT_KEY_MENU_RETURN_TO_GAME",
            activate = OnReturn,
        }),
        BaseMenuItems.Button({
            controlName = "QuickSaveButton",
            textKey = "TXT_KEY_MENU_QUICK_SAVE_BUTTON",
            activate = OnQuickSave,
        }),
        BaseMenuItems.Button({
            controlName = "SaveGameButton",
        ...
        BaseMenuItems.Button({
            controlName = "RestartGameButton",
            textKey = "TXT_KEY_MENU_RESTART_GAME_BUTTON",
            activate = OnRestartGame,
        }),
        BaseMenuItems.Button({
            controlName = "RetireButton",
...
local function leaderLabel()
    ...
    if Game:IsNetworkMultiPlayer() and nick ~= "" then
        return nick
    end
...
local function buildModsItems()
    ...
    for _, m in ipairs(sorted) do
        items[#items + 1] = BaseMenuItems.Text({
            labelText = Text.format("TXT_KEY_CIVVACCESS_GAMEMENU_MOD_ENTRY", m.title, m.version),
        })
    end
    return items
end
