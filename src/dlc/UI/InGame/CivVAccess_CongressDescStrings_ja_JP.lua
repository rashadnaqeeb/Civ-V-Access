-- Mod-authored splash painting descriptions for World Congress special
-- sessions and projects, ja_JP overlay.
-- Baseline in CivVAccess_CongressDescStrings_en_US.lua.
CivVAccess_Strings = CivVAccess_Strings or {}

CivVAccess_Strings["TXT_KEY_CIVVACCESS_CONGRESSDESC_MISSING"] =
    "この画像の説明はありません."

CivVAccess_Strings["TXT_KEY_CIVVACCESS_CONGRESSDESC_LEAGUE_SPECIAL_SESSION_START_WORLD_CONGRESS"] =
    "会議の後方, 聴衆の頭越しに広間の様子が見渡される. 前方では代表たちが赤い布を掛けた長卓を囲んで絨毯敷きの壇上に座り, 各席には白い紙が一枚置かれている. 白いローブと白い頭飾りをまとった一人の人物が卓の上座に立ち, 黒い顎鬚の男を傍らに従えて議場に呼びかけている. 聴衆は木製の椅子の列に向かい合い, ターバンや白帽や何も被らない頭が並んでいる. 右側の壁に沿って白い柱の間からさらに多くの傍聴者が見守っている."

CivVAccess_Strings["TXT_KEY_CIVVACCESS_CONGRESSDESC_LEAGUE_PROJECT_WORLD_FAIR"] =
    "巨大な観覧車が画面右上から突き出し, その縁から密閉された乗客用ゴンドラが吊り下がっている. 車輪の基部は緑のリベット打ちの鉄でできた大きなアーチの背後に下り, そのアーチは FERRIS WHEEL という文字が刻まれた白い壁に架かっている. 下の広い砂埃の遊歩道では博覧会の来場者たちが行き交い, 長いドレスの女性, オーバーコートにボーラーハットやシルクハットの男たちの姿がある. 左手には看板を掲げた低い博覧会の建物が連なり, 遠くには薄青い空の下に白いドーム型の会館がそびえている."

CivVAccess_Strings["TXT_KEY_CIVVACCESS_CONGRESSDESC_LEAGUE_PROJECT_WORLD_GAMES"] =
    "スタートの瞬間, 4人の走者が前傾姿勢でラインを蹴り出し, 頭を低く下げ, 前景を横切る暗いトラックを駆け始める. 左手の奥ではスターターが長い白いコートをまとって立ち, まだ煙の出ている拳銃を掲げている. 背景一面にびっしりと埋まったスタンドは, 色の斑点として群衆を描いている. 右手のトラック沿いには白い梯子形のスタンドと高いポールが並び, その先にはインフィールドの緑が広がっている."
