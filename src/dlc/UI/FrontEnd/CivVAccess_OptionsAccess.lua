...
local function withButtons(items)
    for _, b in ipairs(commonButtons()) do
        items[#items + 1] = b
    end
    return items
end
    ...
    tabs = {
        {
            ...
            items = withButtons({
                ...
                BaseMenuItems.Slider({
                    controlName = "Tooltip1TimerSlider",
        ...
        },
        {
            ...
            items = withButtons({
                ...
                BaseMenuItems.Textfield({
                    controlName = "TurnNotifySmtpUserEdit",
...
local function maybePushGraphicsPopup()
    if Controls.GraphicsChangedPopup and not Controls.GraphicsChangedPopup:IsHidden() then
...
function OnCountdownNo()
    ...
    local wasShowing = Controls.Countdown and not Controls.Countdown:IsHidden()
    ...
    if wasUserAction then
        ...
        SpeechPipeline.speakInterrupt(Text.key("TXT_KEY_CIVVACCESS_OPTIONS_COUNTDOWN_EXPIRED"))
...
function OnOK()
    ...
    if Controls.Countdown and not Controls.Countdown:IsHidden() then
        return
    end
    maybePushGraphicsPopup()
end
