...
### LekMod accessibility gaps — `lekmod-todo.md`
Hand-authored working list of LekMod surfaces that carry no accessibility coverage, so net-new content a re-pin surfaces isn't lost. Entries name the surface, where its code lives, what a player gets today, and what coverage would mean; a closed gap gets its entry deleted, not annotated. Read it when picking up LekMod feature work, and add to it when a re-pin turns up content the layer doesn't read.

### Multiplayer freeze / thread deadlock — `mp-deadlock.md`
Hand-authored. The game-core-thread vs UI-thread lock-order inversion behind the intermittent multiplayer hangs, diagnosed from paired process dumps: what the two locks are, why a Lua callout made from inside `doTurn` deadlocks against UI Lua sitting in an engine binding, the dump evidence that closes the cycle, and which of the mod's surfaces widened a rare base-game race into a common one. Read it before adding a hook that calls Lua from inside the engine, or per-frame Lua that reads engine state. Also records which hooks are deliberately left synchronous and why, the residual risk, and how to reproduce the dump analysis (recovering x86 contexts from a WOW64 dump).
