# Natural Wonders - spoken descriptions

Spoken descriptions for all 17 natural wonders in Brave New World, written from
the game's own 256px atlas portraits (the largest natural wonder art the game
ships; there is no full-screen splash like built wonders have). Style: the
subject first, then palette and light, a third sentence only when a distinctive
flourish earns it. About two to three sentences. The wonder's name is not
repeated because the discovery popup already announces it. Descriptions cover
only what the icon shows; where the artists invented detail for a mythical
place, the description says so plainly. Plain punctuation, no em-dashes.

Shared conventions stated once: every portrait is framed inside a circle, and
the expansion-era icons place a burst of light rays behind the subject. Neither
is repeated per entry.

Each entry: in-game feature Type, English name, then the description text. The
type key is the stable identifier for turning these into mod strings. Note the
non-obvious keys: Krakatoa is FEATURE_VOLCANO, Old Faithful is FEATURE_GEYSER,
The Barringer Crater is FEATURE_CRATER.
