...
| `L` / `Numpad *` | Baseline in-game (directional scan) | Enter Directional Scan mode: read a straight line of tiles outward from the cursor without moving it. Inside the mode `Q` / `E` / `A` / `D` / `Z` / `C` scan the six hex directions (nw / ne / w / e / sw / se), `W` / `X` scan the north / south composite of the two adjacent diagonals, and `Escape` leaves the mode. Each step reports the tile's terrain and owner changes; the ray stops at the map edge or the first unexplored tile, and the closing summary says how far it reached. The line length is set by the directional-scan radius (1–10, default 3), independently of the surveyor radius. | Replaces engine `L` (Lumber mill worker build; Landmark / Holy Site Great-Person builds), all of which fire only for a selected unit / worker and are unreachable through Baseline's letter-capture wall. `Numpad *` is also used by the engine for "next unit needing orders" via `Ctrl+.`/`Numpad *` (`CIV5Controls.xml`), but that binding is swallowed by Baseline's letter-capture wall and is therefore safely reused as a second directional-scan entry chord. A dedicated modal handler repurposes the cursor movement cluster (`Q` / `E` / `A` / `D` / `Z` / `C`) to mean "scan that way" while the mode is active, so the line read never moves the hex cursor. `Ctrl+L` (engine Load, already mod-reassigned to League Overview) is unaffected since this claims only the bare key. |
| `Numpad +` / `Numpad -` | Inside Directional Scan mode | Grow / shrink the directional-scan radius (clamped 1–10, default 3). Announces the new radius; appends "max" or "min" when the limit is reached. | No engine binding on bare `Numpad +` or `Numpad -` inside an active modal context. The engine's global "next city" / "prev city" on these keys is swallowed by the scan mode's own `capturesAllInput`. Keeping radius adjustment inside the mode avoids a modifier conflict with the surveyor's `Shift+W` / `Shift+X`. The directional-scan radius is stored separately from `civvaccess_shared.surveyorRadius` so the two instruments never interfere. |
