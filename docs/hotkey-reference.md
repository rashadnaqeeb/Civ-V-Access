...
| `F11` | Quick save (reassigned to the squad menu on a Community Patch / Vox Populi deploy; see the mod-bound section) |
| `Ctrl+F11` | Quick load (preserved on every deploy via the keycode-only passthrough) |
| `Ctrl+L` | Load game |
| `Ctrl+O` | Options menu |
...
| `\` | Multiplayer Staging Room | Toggle the accessible chat panel (Messages + Compose tabs) | Backslash is unbound by every Civ V XML across base / G&K / BNW (the same chord Baseline uses for in-game chat toggle), so it's free in front-end too. Chat is essential to MP coordination but the engine's chat UI is visual scrolling only, so a dedicated panel with history + compose mode is the minimum viable access path. Panel auto-closes on screen hide. |
...
| `F2` | GreatWorkPopup | Speak a prose description of the displayed painting (great works of art; other work classes speak a short fallback) | Same dead-key situation as the diplo Contexts above: the popup's `BaseMenu` handler captures all input, so `F2` never reached the engine from inside it and Domestic Advisor returns when the popup closes. Sighted players see the artwork splash that fills the popup; the description covers the painting itself. |
| `F2` | NaturalWonderPopup | Speak a prose description of the discovered wonder's portrait art | Same dead-key situation as the diplo Contexts and GreatWorkPopup above: the popup's `BaseMenu` handler captures all input, so `F2` never reached the engine from inside it and Domestic Advisor returns when the popup closes. Sighted players see the wonder's atlas portrait on the popup; the description covers that portrait. |
| `F2` | EndGameMenu | Speak a prose description of the full-screen victory or defeat painting (one per victory type, plus defeat; unrecognized win types show the Score art and describe it) | Same dead-key situation as the popups above: the screen's `BaseMenu` handler captures all input, so `F2` never reached the engine from inside it and Domestic Advisor returns when the screen closes. Sighted players see the painting filling the whole screen behind the buttons; the description covers that painting. |
...
| `Space` | Baseline in-game (selected unit) | Preview the selected unit's route to the cursor tile -- per-tile path geometry, movement-point cost, turn count, fog and embark hints, or a combat preview when an enemy occupies the tile. The same readout `UnitTargetMode` speaks on `Space`, made available without first entering move mode via the Tab action menu or `Alt+M`. Read-only: no move is committed and no two-press latch is armed. Silent when no unit is selected. | Engine's plain `Space` is the unit `Skip turn` mission (`CIV5Missions.xml`), already swallowed by Baseline's `capturesAllInput` and re-exposed on `Alt+X`, so the key is dead at the map layer and free to reuse. Routes through `UnitTargetMode.movePreview`, a thin wrapper over the same `moveModePreview` the in-mode `Space` binding uses, so the spoken result is identical to move mode's first `Space` press. |
...
| `J` / `K` / `L` (`Shift+` reverses) | Scanner in-game | One-key cycle through the first / second / third custom category (name-sorted order, same as the category cycle and the F12 settings list). Each press steps through the category's `all` sub as a single flattened instance list, nearest first, ignoring item grouping; the spoken "X of Y" counts the flat walk, not instances within an item, and no category name is spoken. A cursor away from the snapshot origin (moved by hand, Home, or auto-move) restarts the sweep nearest-first from the cursor; when the cursor is parked on the current entry, the restart steps to the next-nearest instead of re-landing, so an auto-move walk becomes a nearest-neighbor hop. Home / End / Backspace, the directional beep, and auto-move all act on the landed entry exactly as with the PageUp/Down cycles. An unused slot speaks "no custom category N". | A single key has no Alt-instance axis, so grouped instances (two Warriors under one item stop) would be unreachable; the flattened walk is what makes one-key cycling complete. `J` is one of only two letters absent from every engine XML. `K` (Embark / disembark mode, unit die animation, Kasbah build) and `L` (Lumber mill / Landmark / Holy Site builds; `Ctrl+L` load was already reassigned) never fire in map mode anyway — Baseline's `capturesAllInput` wall sits below the scanner handler — so the letters are de-facto free there. |
| `Home` / `M` | Scanner in-game | Jump the hex cursor to the current scanner entry's plot; remembers the pre-jump cell for Backspace. Speaks "here" when the cursor is already on the entry's plot rather than re-running the full glance. `M` is a same-action alias of `Home`. | Replaces `Next city`. The scanner's Cities category with owner subcategories is a strict superset of the engine's city cycle; losing it costs nothing. Routes through `ScannerNav.jumpCursorTo`, the shared mark-then-jump primitive used by Bookmarks Shift+digit and Ctrl+S jump-to-capital -- so the at-target SCANNER_HERE short-circuit and the Backspace anchor capture are common across all three jumps. The `M` alias puts the jump on the home row next to the `J` / `K` / `L` custom-category cycle so a cycle-then-jump sequence stays under one hand. Engine `M` (Move mode, Manufactory build) never fires in map mode — Baseline's `capturesAllInput` wall sits below the scanner handler — and the mod's own `Ctrl+M` (map settings) and `Alt+M` (move target picker) live on different chords, so bare `M` is de-facto free there. |
...
| `Ctrl+Up` / `Ctrl+Down` / `Ctrl+Left` / `Ctrl+Right` | Scanner in-game | Scope the scanner to a 90-degree compass arc (north / south / west / east) fanning out from the hex cursor, so only entries in that direction surface; pressing the active direction again clears the scope. Opt-in: inert unless the F12 `Scanner can limit entries to one direction` toggle is on, which also decides whether the chord is listed in the `?` help. | Replaces the engine's bare-arrow world-camera pan (Ctrl+arrow itself carries no engine binding, and pan has no value to a blind player). Arrows are the natural compass affordance, and WorldView's input hook already routes Ctrl+arrows through the HandlerStack before the camera pan, returning true on a hit so the camera never moves. The bindings register regardless of the toggle -- Baseline's `capturesAllInput` wall swallows Ctrl+arrows either way, so `ScannerNav.setDirection` reads the toggle live and returns nothing when off, which keeps a mid-session flip effective. |
| `Ctrl+1` through `Ctrl+0` | Bookmarks in-game | In map mode, save the cursor's current cell into the matching slot (1-9, 0), speaking "bookmark added". In unit mode (see `Ctrl+B`), bookmark the selected unit instead, speaking "bookmarked <unit>" or "no unit selected". Subsequent saves to the same slot overwrite. | No engine binding on any modifier-combined top-row digit (confirmed absent from all CIV5 XML). Slots persist across save/load via `Modding.OpenUserData` (a per-machine SQLite store scoped by map seed and active player) -- outside the `.Civ5Save` and the engine's sync archive, so MP clients keep separate slots and asymmetric mod loading can't desync. |
| `Shift+1` through `Shift+0` | Bookmarks in-game | In map mode, jump the hex cursor to the saved cell, speaking the destination glance the same way a cursor move would; empty slots speak "no bookmark", the cursor already on the cell speaks "here". In unit mode, select the bookmarked unit and bring it into view (the `cursorFollowsSelection` setting governs the cursor jump, same as the Military Advisor); empty slots speak "no unit bookmark", a unit that has died / disbanded / been given away speaks "unit no longer exists". | No engine binding on any modifier-combined top-row digit. The map jump routes through `ScannerNav.jumpCursorTo`, the shared mark-then-jump primitive used by scanner Home / Ctrl+S jump-to-capital; the unit jump routes through `UnitControl.selectAndReveal`, shared with the Military Advisor row activation. |
| `Alt+1` through `Alt+0` | Bookmarks in-game | Speak distance and direction from the cursor to the slot's target -- the saved cell in map mode, the unit's live plot in unit mode -- plus the capital-relative coord when the scanner-coord setting is on. Empty slots speak "no bookmark" / "no unit bookmark". | No engine binding on any modifier-combined top-row digit. Direction format is byte-identical to the scanner's `End` readout (shared `HexGeom.directionString`); the coord segment reads `civvaccess_shared.scannerCoords` so users get one consistent vocabulary across scanner and bookmark spatial queries. |
| `Ctrl+B` | Bookmarks in-game | Toggle the digit chords between map bookmarks (cursor cells) and unit bookmarks (the player's own units). Announces the new mode ("map bookmarks" / "unit bookmarks") on every press. Resets to map mode on every game load. | Engine's plain `B` is `Ranged attack` / `Found city` / Brazilwood build (all `AltDown=0 ShiftDown=0` with no `CtrlDown`), so `Ctrl+B` is unbound, and Baseline's `capturesAllInput` swallows the chord regardless. Mode is invisible state, so the per-press announcement is the only orientation a blind player gets; map and unit save / jump announcements also differ in content as the backstop. Unit slots resolve `{owner, id}` live via `Players[owner]:GetUnitByID` at jump time -- a stale unit reads as "unit no longer exists", never a wrong location. |
...
| `Ctrl+Shift+E` | Baseline in-game (Vox Populi only) | Open the Community Patch Events Overview popup (`BUTTONPOPUP_MODDER_6`) with an accessible four-tab wrapper (Player Choices, City Choices, Player Events, City Events — all four list active effects; Enter on a city row drops the hex cursor on the city, and each tab carries the vendor panel's Sort By pulldown). `Data1=1` toggles closed when already visible. Speaks "Events are disabled in this game" when none of the five VP event game options is on. | Registers only when the CP DLL is present (capability probe; the vanilla DLL also registers the `MODDER_*` popup enums, so the enum table is not a discriminator). Shift joins the two E chords next to `Ctrl+E` Espionage. No engine binding on `Ctrl+Shift+E` in any CIV5 XML. |
| `Ctrl+V` | Baseline in-game (Vox Populi only) | Open the Community Patch Vassal Overview popup (`BUTTONPOPUP_MODDER_11`) with an accessible two-tab picker / details wrapper (your vassals as a master, or your master's players as a vassal; details cover statistics, taxation with the tax slider, treatment, independence conditions, and the Liberate / Request Independence actions behind the vendor's Yes/No confirm). `Data1=1` toggles closed when already visible. Speaks "Vassalage is disabled in this game" when `GAMEOPTION_ENABLE_VASSALAGE` is off. | Registers only when the CP DLL is present (same capability gate as `Ctrl+Shift+E`). The only engine binding on the V key is bare `V` (`CONTROL_ADVISOR_COUNSEL`, already swallowed by Baseline's letter wall and reassigned to `F10`); no CIV5 XML binds `Ctrl+V`, so the chord is free. Mnemonic: V for vassals. |
| `Ctrl+M` | Baseline in-game | Open the accessible Map settings menu: hex grid, yield icons, resource icons, trade routes, hide tile recommendations, strategic view on/off, plus the strategic-view sub-settings (features, fog, overlay mode, icon mode) when strategic view is active. Each item reads live state (`OptionsManager.Get*On` / `InStrategicView` / `GetMapOptions`) and writes through the minimap Map Options panel's own check / selection handlers, mirroring the visible checkbox. Opens via the `LuaEvents.CivVAccessMapSettingsToggle` bridge into MiniMapPanel's env, where those handlers live. | Replaces engine `Ctrl+M` (`INTERFACEMODE_MOVE_TO_SAME_TYPE`, the Shift+click move-same-type-units-on-plot mode), which is mouse-first and already swallowed by Baseline's letter wall. The engine's own map-overlay hotkeys (`G` grid, `Y` yields, `Ctrl+R` resources, `F10` strategic view) are blocked as noise under Baseline, so this menu is the keyboard path to those overlays plus the flyout-only trade-route / recommendation toggles. Mnemonic "M for map"; distinct from `Alt+M` (Move To) under `(key, mods)` dispatch. Unit icons (engine `Alt+U`) are intentionally absent: no Lua toggle path exists (empty gamecore `CONTROL_UNIT_ICONS` stub), and `Alt+U` is already the mod's Upgrade-unit command. |
| `Ctrl+N` | Baseline in-game | Name (or rename) the landmass or ocean at the cursor's tile: speaks a prompt, captures typed letters / digits / spaces (screen reader provides the typing echo), Enter commits, Escape or an empty commit cancels. The name is spoken when the cursor crosses onto the mass and replaces the "Landmass N" ordinal in the scanner's geography category. On an unexplored tile speaks "unexplored"; on a lake speaks "no landmass or ocean here" (lakes read as terrain, not bodies of water). Names persist per map seed and player via ModUserData, outside the savefile and the MP mod-hash. | Replaces nothing: no CIV5 XML binds `Ctrl+N` (bare `N` is Mine / Nuclear strike, both already swallowed by Baseline's letter wall). Sits next to `Alt+N` (rename unit), keeping the rename chords on one mnemonic. |
| `F11` | Baseline in-game (Community Patch / Vox Populi only) | Open the squad menu: the orchestrator lists the player's squads plus "add new squad"; Enter on a squad opens its editor (live member rows where Enter removes a unit, an escort-civilians toggle, a wake-mode cycler, a rename field, and delete). | Replaces engine `Quick save`. Plain F11 stays in `passthroughKeys`, so the plain-F11 binding wins at the bindings loop (the same way plain F10 wins over Ctrl+F10) while `Ctrl+F11` (Quick load) still reaches the engine via the keycode-only passthrough. Quick-save is lost on a CP-DLL deploy; it stays reachable through the Esc menu and is unaffected on vanilla / LekMod, where the binding never registers. Squads is a Vox Populi feature with no vanilla equivalent, so the layer is gated on `squadsAvailable()`. |
| `Up` / `Down` | Baseline in-game (Community Patch / Vox Populi only) | Cycle the focused squad. Speaks the squad's movement status ("moving, N turns left") and stays silent when the squad is idle; an empty roster speaks "no squads". Changes neither the unit selection nor the cursor. | Arrows reach Baseline through the WorldView input hook, which dispatches the HandlerStack before the engine camera pan (the same path the scanner's Ctrl+arrows use). On vanilla / LekMod the squad bindings never register, so plain arrows keep their prior swallowed-by-Baseline behavior. The squad name is reserved for the action keys, so the cycle scan is a fast "is this one moving" sweep. |
| `Left` / `Right` | Baseline in-game (Community Patch / Vox Populi only) | Cycle the focused unit within the focused squad, speaking its row (name, direction and distance from the hex cursor, HP if hurt, moves, status). An empty squad speaks "no units". No selection or cursor change. | Same arrow-routing rationale as Up/Down. Read-only inspection of the squad's membership without disturbing the player's current selection. |
| `Alt+/` | Baseline in-game (Community Patch / Vox Populi only) | Select the focused squad member (the unit `Left`/`Right` lands on, the one `Alt+Left` would remove) as the engine head selection, so the player can issue normal unit commands to it. Speaks the selection via the unit-selection listener; an already-selected unit only re-centers and speaks its info. No squad speaks "no squads"; an empty squad speaks "no units". | Bridges the otherwise-decoupled mod-side squad focus to the engine selection, the same `selectAndReveal` path the unit bookmarks and Military Advisor use. Rides the `/` (`VK_OEM_2`) unit-info key, which already carries plain `/` info and `Ctrl+/` recenter; `Alt+/` is the free chord and the KeyLayout remap keeps it on the unit-info keycap across national layouts. |
| `Alt+Left` | Baseline in-game (Community Patch / Vox Populi only) | Remove the focused unit from its squad; speaks "removed from <squad>". When the focused squad is already empty, deletes the squad outright instead and speaks "<squad> deleted". | Alt-chorded arrows arrive as `WM_SYSKEYDOWN`, which the InputRouter already folds in alongside `WM_KEYDOWN`. The empty-squad delete is the structural mirror of Alt+Right creating a squad when none exist: each membership key does the structural op in the degenerate state. |
| `Alt+Right` | Baseline in-game (Community Patch / Vox Populi only) | Add the head-selected unit to the focused squad and move focus onto it (so an immediate Alt+Left removes it); speaks "added to <squad>". A unit already in another squad is reassigned with "moved to <squad>". When no squads exist yet, creates one, adds the unit, and opens the new squad's editor so its settings can be set right away. No unit selected speaks "no unit selected". The squad's wake mode is re-pushed to the new member. | Mirrors Alt+Left as the add half of the membership pair. |
| `Alt+Up` | Baseline in-game (Community Patch / Vox Populi only) | Enter the move sub-mode for the focused squad (speaks the squad name on entry). In the sub-mode the cursor roams; Space previews the turns for the whole squad to reach the cursor tile, Enter sends the squad there with its escort setting, Esc cancels. When the squad is already mid-move, the first Alt+Up instead reads the move (movement state, turns left, and destination bearing); a second Alt+Up on the same still-moving squad cancels that move and reopens the sub-mode to pick a new one. No squads speaks "no squads". | The sub-mode is UnitTargetMode-shaped with `capturesAllInput=false`, so the cursor and info queries still fall through to Baseline. The read-then-reissue two-press arms on the squad identity and clears on any other squad action (the combat-confirm latch shape), so navigating away can't leave a stray reissue primed; this folds the old Alt+Down report-and-cancel into the move key so changing a squad's destination is read, press, pick rather than three separate keys. |
| `Alt+Down` | Baseline in-game (Community Patch / Vox Populi only) | Open the focused squad's editor directly, the same per-squad menu reached by Enter on a row of the F11 list: live member rows (Enter removes a unit), an escort-civilians toggle, a wake-mode cycler, a rename field, and delete. No squad speaks "no squads". | Skips the F11 orchestrator so the player edits the squad they are already focused on in one press. The editor stands alone off the handler stack; its delete path guards the orchestrator-list refresh on the orchestrator still being open. |
| `Ctrl+Space` / `Ctrl+Enter` | Baseline in-game | End turn. On no blocker, calls `CONTROL_ENDTURN`; the ActivePlayerTurnEnd listener announces "Turn ended" when the engine confirms. On a blocker, announces the blocker label (same TXT_KEY the engine's end-turn button displays) and dispatches the engine's take-me-to-the-blocker action (`UI.ActivateNotification` for screen blockers; `UI.SelectUnit` + `UI.LookAt` for unit / stacked-unit / promotion / range-attack blockers). In multiplayer, pressing after submission un-readies the turn and announces "Waiting for players". | Overrides the engine's vanilla `Ctrl+Space` alt-end-turn (`CIV5Controls.xml`) so the announcement pipeline can own every end-turn attempt. The Baseline capture-by-default also swallows `Return` / `Numpad Enter` / `Shift+Return`, the engine's primary and force-end bindings, so `Ctrl+Space` and `Ctrl+Enter` are the only end-turn paths from the map — no silent engine bypass. `Ctrl+Enter` (and `Ctrl+Numpad Enter`, same keycode) mirrors `Ctrl+Space` so the end-turn pair stays consistent with the force-end pair below; both chords route to one dispatcher. |
| `Ctrl+Shift+Space` / `Ctrl+Shift+Enter` | Baseline in-game | End turn past the units-need-orders prompt, and past the Social Policy prompt when the game was set up with Allow Policy Saving. On `ENDTURN_BLOCKING_NONE` or `ENDTURN_BLOCKING_UNITS`, calls `CONTROL_FORCEENDTURN` (the ActivePlayerTurnEnd listener announces "Turn ended"). On `ENDTURN_BLOCKING_POLICY` with `GAMEOPTION_POLICY_SAVING` set, calls `UI.RemoveNotification` on the blocking index to bank the culture, announces "Policy skipped, culture kept", then re-reads the blocker and continues with whatever sat behind the prompt. For any other blocker — research, production, promotion, religion, votes, ideology, archaeology, stacked units, city range attack, steal tech, and policy with the option off — falls through to the same announce-and-open path as `Ctrl+Space`, because the engine's `CONTROL_FORCEENDTURN` handler at `CvGame.cpp:3712` is gated on `(eBlock == NO_ENDTURN_BLOCKING_TYPE \|\| eBlock == ENDTURN_BLOCKING_UNITS) && allAICivsProcessedThisTurn() && allUnitAIProcessed()` and silently no-ops otherwise. | Mirrors the engine's `Shift+Return` (same semantics, not "bypass everything" despite the common "force end" label). Two-modifier chord is deliberate friction against fatfinger force-end over plain `Ctrl+Space`; the policy dismiss rides it for the same reason, since banking culture suppresses the policy prompt until the next policy is adopted (`SetPolicyNotificationSeen`), so it must not sit on the key pressed every turn. The dismiss is the keyboard seat for base `ActionInfoPanel.lua`'s right-click on the End Turn button, the game's only path to saving a policy; `GAMEOPTION_POLICY_SAVING` does not stop the blocker being raised, it only makes the notification user-dismissable (`CvNotifications::MayUserDismiss`). `Ctrl+Shift+Enter` (and `Ctrl+Shift+Numpad Enter`, same keycode) routes to the same `forceEndTurn` dispatcher because `Ctrl+Shift+Space` is intercepted before the game window ever sees it on machines where another app, such as 1Password (whose Quick Access shortcut defaults to `Ctrl+Shift+Space`), has claimed it as a global hotkey via `RegisterHotKey`. |
...
| `Alt+X` | Baseline in-game (selected unit) | Skip the unit's turn (`MISSION_SKIP`). | Replaces nothing — engine's plain `Space` is `MISSION_SKIP` but Baseline doesn't pass it through, and `Alt+Space` is consumed by Windows as the system-menu accelerator before the game's input handler sees it. `Alt+X` is unbound by the engine and plain `X` is the mod's combat-details readout, so the chord is conflict-free. |
...
| `Alt+Up` / `Alt+Down` | BaseMenu items (menus, pickers, popups), BaseTable cells (F2/F3-style tables), and the tech tree tree/grid tab | Walk the focused item's announcement one content section at a time. For a menu item / table cell: each control part (row label, column, cell value), then each deduped tooltip sentence. For a tech: name, status, turns, then each unlocks line ("Cost", "Leads to", "Allows you to build"). Section boundaries come from the engine's own structure (control parts plus raw `[NEWLINE]`), not statistical sentence splitting. Walking off either end re-speaks the edge section. The verbose position / kind tags are excluded. | No engine binding on either combo inside a captured screen, and `Alt+Left` / `Alt+Right` are already the back/forward hooks, leaving the vertical pair free. The navigator lives in `BaseMenuItems.SectionReview` (the module included alongside every menu / table / tech screen) so all three surfaces share one implementation. Added for Vox Populi, whose tooltips make a single item's full readout (e.g. a building in the production chooser, or a tech's unlocks) too long to parse in one utterance. |
| `Alt+Home` / `Alt+End` | Same three surfaces as `Alt+Up` / `Alt+Down` | Jump the section reviewer to the focused item's first / last section and leave the cursor seated there, so a following `Alt+Down` / `Alt+Up` resumes from that end. Same section list, same exclusions. | The Alt variants of the keys that already mean first / last for the item list, so the modifier keeps its one meaning across the screen: bare key walks items, Alt walks the focused item's content. Dispatch is exact-`(key, mods)`, so the plain first-item / last-item bindings are untouched. Earns its own chord because a long Vox Populi tooltip can run a dozen sections, where the last one (typically the cost or requirement line) is what the player is after. |
...
- **Letters:** `J`, `X` (only these two are truly absent from all XML; both now bound by the mod — `X` for combat details, `J` for the custom-category cycle alongside `K` / `L`). `G` is not in the XML either but is widely reported to toggle the hex grid — treat as unverified / probably engine-hardcoded.
