...
| `Numpad +` / `Numpad -` / `+` / `-` | Inside Directional Scan mode | Grow / shrink the directional-scan radius (clamped 1–10, default 3). Announces the new radius; appends "max" or "min" when the limit is reached. | No engine binding on the key chords inside the modal context. The top-row aliases keep the radius control usable on keyboards without a numpad, while the numpad chords preserve the original feel on full keyboards. The directional-scan radius is stored separately from `civvaccess_shared.surveyorRadius` so the two instruments never interfere. |
