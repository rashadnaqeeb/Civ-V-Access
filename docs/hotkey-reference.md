...
| `Shift+T` | Baseline in-game | Intentionally absent. | Bare `T` already speaks the Maya long-form date when relevant, which is the only thing `CurrentDate`'s tooltip would have added. No additive content for a Shift extension to speak. |
