...
| `S` | Baseline in-game | Speak distance and direction from the player's capital | Replaces `Set up for ranged attack`, `Air strike`, `Scrub fallout` -- all unit/worker missions that fire only when a relevant unit is selected. Spatial orientation is the cursor's defining feature. |
...
| `Shift+C` | Baseline in-game (surveyor) | List hills in the radius, one `hill <direction>` per instance, period-separated | No engine binding on `Shift+C` (engine's `Ctrl+C` is Select-of-type / Citadel; `Alt+C` is Select-all / Remove-jungle). The repeated `hill` label is the price of unambiguous instance boundaries when compound directions contain commas. |
| `Shift+S` | Baseline in-game (surveyor) | List cities in the radius, grouped friendly / hostile / neutral | No engine binding on `Shift+S`. Replaces an earlier cursor-recenter binding (dropped; Cursor.recenter and its TXT keys are gone) because the group-by-diplomacy listing serves the same "find your units and cities" need more completely than a one-shot recenter. |
