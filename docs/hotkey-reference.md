...
| `Alt+Left` | Baseline in-game (Community Patch / Vox Populi only) | Remove the focused unit from its squad; speaks "removed from <squad>". | Alt-chorded arrows arrive as `WM_SYSKEYDOWN`, which the InputRouter already folds in alongside `WM_KEYDOWN`. |
| `Alt+Right` | Baseline in-game (Community Patch / Vox Populi only) | Add the head-selected unit to the focused squad and move focus onto it (so an immediate Alt+Left removes it); speaks "added to <squad>". No unit selected speaks "no unit selected"; no squad speaks "no squads"; a unit already in another squad is reassigned with "moved to <squad>". The squad's wake mode is re-pushed to the new member. | Mirrors Alt+Left as the add half of the membership pair. |
| `Alt+Up` | Baseline in-game (Community Patch / Vox Populi only) | Enter the move sub-mode for the focused squad (speaks the squad name on entry). In the sub-mode the cursor roams; Space previews the turns for the whole squad to reach the cursor tile, Enter sends the squad there with its escort setting, Esc cancels. | The sub-mode is UnitTargetMode-shaped with `capturesAllInput=false`, so the cursor and info queries still fall through to Baseline. The squad name is spoken here because the action keys carry the confirmation the silent Up/Down scan omits. |
| `Alt+Down` | Baseline in-game (Community Patch / Vox Populi only) | Report the focused squad's full status (name, unit count, movement state, wake mode, escort when on). When a move is in progress, a second Alt+Down on the same squad cancels it (armed two-press); the cancel is only armed while a move is actually running. | The two-press cancel keys on the squad identity rather than a timer, the same shape as the combat-confirm latch: any other squad action disarms it, so the player cannot accidentally cancel a squad they navigated away from. |
