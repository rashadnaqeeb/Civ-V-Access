...
| `G` | Baseline in-game | Speak gold per turn, total, used / available trade routes, and any strategic-resource shortage. Format: `+<rate> gold, <total> total, <used> of <avail> trade routes` (or `minus <rate> gold, ...` when `CalculateGoldRate` is negative), suffixed with `, no <resource>` per shortage when `GetNumResourceAvailable < 0`. | No engine binding on plain `G` (no XML row claims it). Folds three top-panel readouts (gold, trade routes, strategic resources) onto one key because they're all economic-flow indicators the player checks at the same point in a turn. |
| `H` | Baseline in-game | Speak empire happiness then golden-age state, in that fixed order regardless of GA active vs progressing. Happiness branch: `+<excess> happiness` / `Unhappy minus <deficit>` / `Very unhappy minus <deficit>` (driven by `IsEmpireUnhappy` / `IsEmpireVeryUnhappy`). GA branch: `golden age for <turns> turns` when `GetGoldenAgeTurns > 0`, else `<cur> of <threshold> to golden age`. Honours `NO_HAPPINESS` (`Happiness off`). | Replaces engine's `H` mission cluster (Heal mission / Camp / Customs House build / Select-healthy Ctrl-control), all narrow / contextual / unreachable through Baseline's letter wall. Excess happiness is what fuels golden-age progress, so the two readouts share the key. Fixed order at user request — predictable shape on every press. |
| `F` | Baseline in-game | Speak `+<rate> faith, <total> total`. Honours `NO_RELIGION` (`Religion off`). | Replaces engine's `F` mission cluster (Wake command / Sleep-Fortify-Garrison mission / Fishing Boats build / Fort Ctrl-build / Fishing Boats No Kill BNW build), already swallowed by Baseline. We re-expose Wake / Fortify / Sleep on `Alt+F` / `Alt+W`, leaving plain `F` open for the empire-status readout that matches the letter mnemonically. |
...
| `Alt+F` | Baseline in-game (selected unit) | Fortify a military unit, sleep a civilian. Routes through `UnitActionMenu.commitByType` against `MISSION_FORTIFY` then `MISSION_SLEEP`, so the engine picks by unit type the same way plain `F` does in vanilla. | Plain `F` is the engine's wake / sleep / fortify / garrison hotkey, but Baseline's letter wall swallows it. Alt-modifier preserves the mnemonic. No engine binding on `Alt+F`. |
| `Alt+S` | Baseline in-game (selected unit) | Sentry — sleep until an enemy comes into sight (`MISSION_ALERT`). | Replaces engine's `Alt+S` (`Air Sweep` interface mode, only fires for an air unit with sweep). Mnemonic for "sentry"; the engine's plain `A` for Alert is unreachable through Baseline's letter wall and is also our cursor-direction key. |
