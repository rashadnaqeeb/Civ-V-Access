...
| `Up` / `Down` | Baseline in-game (Community Patch / Vox Populi only) | Cycle the focused squad. Speaks the squad's movement status ("moving, N turns left") and stays silent when the squad is idle; an empty roster speaks "no squads". Changes neither the unit selection nor the cursor. | Arrows reach Baseline through the WorldView input hook, which dispatches the HandlerStack before the engine camera pan. On vanilla / LekMod the squad bindings never register, so plain arrows keep their prior swallowed-by-Baseline behavior. The squad name is reserved for the action keys, so the cycle scan is a fast "is this one moving" sweep. |
