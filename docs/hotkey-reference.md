...
| `Ctrl+Shift+Space` | Baseline in-game | Force end turn (`CONTROL_FORCEENDTURN`), ignoring blockers. The ActivePlayerTurnEnd listener still announces "Turn ended". | Mirrors the engine's `Shift+Return` force-end but under a harder-to-mistouch chord. Plain `Ctrl+Space` would be one modifier away from plain `Space` and too easy to fatfinger force-end; the two-modifier form is deliberate friction. |
