...
Status (Allied, Friend, Neutral, Afraid, Angry, At War), active quest list, current ally if any, type and personality, shared resources if allied, influence value and resting point. The action stack — Peace, Give, Pledge to Protect, Revoke, Take (bully), War, Stop Unit Spawning, Buyout — is fully wired. Gift submenu (small / medium / large gold, gift unit, gift improvement) and bully submenu (gold tribute, unit tribute) both walk; bully-confirm sub-popup is read. Gift-unit and gift-improvement, which dismiss the popup and set INTERFACEMODE_GIFT_UNIT / INTERFACEMODE_GIFT_TILE_IMPROVEMENT (hex-click-only modes), are wired through GiftMode — a CityRangeStrikeMode-shaped sibling pushed from the popup's commit. Cursor roams freely; Space speaks `CanDistanceGift` (unit) or `CanMajorGiftTileImprovementAtPlot` (improvement) plus the plot glance, falling back to "cannot gift here" on illegal targets; Enter commits via the same engine flow the hex click would (`BUTTONPOPUP_GIFT_CONFIRM` for unit, `Game.DoMinorGiftTileImprovement` for improvement); Esc cancels.
