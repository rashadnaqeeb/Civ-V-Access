...
Bug fixes:
- Puppet cities can now perform their once-per-turn ranged strike. Pick View City from the annex prompt, open the city hub, and Ranged Strike appears whenever the city has a valid target. Previously the option was suppressed during the viewing-mode peek and the mod had no other path to it, even though sighted players had it via the floating banner button.
