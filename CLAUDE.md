...
- `resync-vp.ps1` — re-pins CP/VP support to a new upstream release in resumable phases (engine / mods / vendor / finish / modpack / cp-modpack): rebase the fork, rebuild, gate on the DLL canary, mirror MODS to the clone, re-run vendor verify + generate (both `--engine vp` and `--engine cp`), then stage the modpack bake (`stage-vp-modpack-bake.ps1`), record the new pin, and bump the component versions the re-pin moved (`engine_vp`, the VP/CP overlays, `vp_runtime` when the substrate changed; via `tools/component-versions.ps1`, once per release cycle); the terminal `modpack` phase (run after a manual CP+VP merged-DB session) bakes the VP modpack, bumps `vp_modpack`, and deploys via `deploy.ps1 -State vp`, and `cp-modpack` (after a manual CP-only session) does the same for the Community-Patch-only modpack (`cp_modpack`, `deploy.ps1 -State cp`). See `docs/llm-docs/cp-vp-support.md` for the runbook.
