...
- `build-modpack.ps1` — bakes CP and VP into a single DLC modpack via `tools/modpack/`. It dumps the engine-merged cache DB (the byproduct of a normal modded session) to the merged Override XML, embeds the CP/VP mod folders plus the fork DLL, and writes the manifest. It reads the already-merged DB and never re-implements merge semantics.
- `deploy-modpack.ps1` — installs the VP modpack state: proxy, Tolk, cinematics, the baked modpack package, and our DLC built modpack-shaped (full VP-bodied vendor overlay folded into the DLC, no MODS overlay, fork embedded in the package). The third exclusive install state alongside `deploy.ps1` and `deploy-vp.ps1`.
...
- `resync-vp.ps1` — re-pins VP support to a new upstream release in resumable phases (engine / mods / vendor / finish / modpack): rebase the fork, rebuild, gate on the DLL canary, mirror MODS to the clone, re-run vendor verify + generate, then deploy the mod-overlay and record the new pin; the terminal `modpack` phase (run after a manual merged-DB session) bakes and deploys the modpack so the install lands back in player-facing modpack state. See `docs/llm-docs/vox-populi.md` for the runbook.

The VP and modpack scripts depend on the sibling Community-Patch-DLL clone and the staged vendor tree; the seam, vendoring, and re-sync details live in `docs/llm-docs/vox-populi.md`. The committed pin is `supported_vp` in `versions.json`.
...
- `docs/llm-docs/` — derived reference material (Lua API per class, Events / LuaEvents catalogs, screen inventory, external resource pointers) plus the Vox Populi support reference (`vox-populi.md`). Load on demand. See `docs/llm-docs/CLAUDE.md` for what's in each file and when to consult it.
...
- `src/vp/` — the Vox Populi body of `CivVAccess_EngineData.lua`, swapped in under the shared include stem by the VP deploys. The only source file that differs by engine; everything else is shared and feature-detects at runtime. See `docs/llm-docs/vox-populi.md`.
...
- `tools/vendoring/` — `vendor.py` + `manifest.json`, which model every vanilla vendor override and generate the per-engine override trees. `verify` reproduces the committed vanilla tree byte-for-byte; `generate --engine vp` stages the VP tree. Do not hand-edit a generated override; edit its manifest entry and regenerate.
...
The override tree is generated, not hand-edited. `tools/vendoring/manifest.json` models each override as prefix plus verbatim engine body plus byte-exact edit recipes plus appended include, with per-engine `vp` sub-objects where the CP/VP body differs from vanilla. `vendor.py verify` reproduces the committed vanilla tree byte-for-byte (the regression gate); `generate --engine vp` stages the VP tree. Do not hand-edit a generated override — change its manifest entry and regenerate. Details in `docs/llm-docs/vox-populi.md`.
