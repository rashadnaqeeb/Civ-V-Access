...
The same accessibility layer also targets Vox Populi (the Community Patch / Vox Populi balance overhaul). VP support reuses the whole vanilla layer and swaps the engine fork for a fork of Community-Patch-DLL, routes every engine-divergent read through an EngineData seam, and ships either as a mod overlay or as a baked DLC modpack. A Community-Patch-only modpack (the bug-fix/AI DLL without the Vox Populi balance overhaul) is also supported, reusing the same seam body, which feature-detects the balance mode at runtime, so VP-balance surfaces gate on `Game.IsCustomModOption("BALANCE_VP")` rather than bare CP-DLL presence. An install is in exactly one of several mutually exclusive states (vanilla, VP mod-overlay, VP modpack, CP modpack); the running session type must match the last deploy, or the player hears silent wrong numbers. The full VP layer is documented in `docs/llm-docs/cp-vp-support.md`; read it before touching VP code, the seam, the vendoring tool, or the deploy scripts.
...
- `deploy.ps1` — copies the committed artifacts into the game install: `dist/proxy/lua51_Win32.dll`, the Tolk runtime DLLs from `third_party/tolk/dist/x86/`, the `src/dlc/` payload into `Assets/DLC/DLC_CivVAccess/`, `sounds/*.wav` into the DLC's `Sounds/`, and `dist/engine/CvGameCore_Expansion2.dll`. The engine DLL is always deployed (the ~4 MB copy is cheap, and skipping it would leave a modpack's embedded fork DLL in place when flipping back to vanilla — silent wrong numbers); on first install it backs up the vanilla DLL to `build/CvGameCore_Expansion2.vanilla.dll.bak`. `-Uninstall` reverses everything (restoring the vanilla engine DLL if a backup exists).
...
- `deploy-vp.ps1` — the Vox Populi variant of `deploy.ps1`. Same proxy / Tolk / DLC / cinematics, plus: overlays the staged VP vendor overrides from `build/vendor/vp` (run `py tools/vendoring/vendor.py generate --engine vp` first), swaps in `src/vp/CivVAccess_EngineData.lua` under the shared include stem, and places our Community-Patch-DLL fork from `dist/engine-vp/` (committed; built on the sibling clone's `civvaccess` branch via `build_vp_clang_sdk.py`) into `MODS\(1) Community Patch\`, where VP loads its DLL. Completes a partial plain-VP install from the clone when pieces are absent. A VP-deployed install is not safe for vanilla sessions (the overlaid vendor files call VP-only bindings); `./deploy.ps1` flips back. The mod-overlay is now a maintainer-only build step (generating the merged DB during a re-pin), not a play/test state: it refuses to install without `-RepinBuild` (`-Uninstall` exempt), since its saves cannot load under the modpack and the wrong state crashes on load. Players and testers use `deploy-modpack.ps1`. `deploy-vp-sighted-multiplayer.ps1` is the sighted-partner variant (fork DLL + empty-DLC manifest on a machine that already has VP installed).
- `build-modpack.ps1` — bakes CP and VP into a single DLC modpack via `tools/modpack/`. It dumps the engine-merged cache DB (the byproduct of a normal modded session) to the merged Override XML, embeds the CP/VP mod folders plus the fork DLL, and writes the manifest. It reads the already-merged DB and never re-implements merge semantics. `-CommunityPatchOnly` bakes a Community-Patch-only pack instead (CP mod alone, CP addin set, distinct GUID, from a clean CP-only cache with BALANCE_VP off), writing to `build/modpack-cp-out`.
- `deploy-modpack.ps1` — installs the VP modpack state: proxy, Tolk, cinematics, the baked modpack package, and our DLC built modpack-shaped (full VP-bodied vendor overlay folded into the DLC, no MODS overlay, fork embedded in the package). One of four exclusive install states (`deploy.ps1` vanilla, `deploy-vp.ps1` VP mod-overlay, this for the VP modpack, plus the CP modpack). `-CommunityPatchOnly` deploys the Community-Patch-only modpack instead (cp vendor stage `build/vendor/cp`, package at `Assets/DLC/ZCivVAccessCP`, CP addin set, no plain-VP completion); the EngineData seam body is shared with VP and feature-detects BALANCE_VP at runtime.
- `build-modpack-cp.ps1` / `deploy-modpack-cp.ps1` — thin wrappers over the two scripts above with `-CommunityPatchOnly`, so the CP and VP modpack bake/deploy share one implementation and cannot drift across a re-pin. CP-only is a beta state: events have no data (the event framework is in the CP DLL but the events themselves are VP content), and the happiness model reverts to the vanilla surplus model (the seam and wrappers gate VP-balance surfaces on `Game.IsCustomModOption("BALANCE_VP")`, not bare CP-DLL presence).
- `package-modpack-bundle.ps1` — maintainer-run; zips the non-committed inputs a tester needs so they can run `deploy-modpack.ps1` with no clone and no build. Re-issue only on a VP re-pin or fork rebuild.
- `resync-vp.ps1` — re-pins CP/VP support to a new upstream release in resumable phases (engine / mods / vendor / finish / modpack / cp-modpack): rebase the fork, rebuild, gate on the DLL canary, mirror MODS to the clone, re-run vendor verify + generate (both `--engine vp` and `--engine cp`), then deploy the mod-overlay and record the new pin; the terminal `modpack` phase (run after a manual CP+VP merged-DB session) bakes and deploys the VP modpack, and `cp-modpack` (after a manual CP-only session) does the same for the Community-Patch-only modpack so it stays current too. See `docs/llm-docs/cp-vp-support.md` for the runbook.

The LekMod scripts follow the same shape, lighter (an easier VC9 toolchain, one install state, no bake):

- `build-engine-lekmod.ps1` — compiles the LekMod engine fork from the sibling LekMod clone's `civvaccess` branch (`LEKMOD_DLL/`) with VC9 / SDK 7.0, output `dist/engine-lekmod/CvGameCore_Expansion2.dll` (committed). NQMod lineage, so it reuses the vanilla fork's toolchain, not VP's clang build.
- `deploy-lekmod.ps1` — the single LekMod state (DLC overlay; no bake). Installs LekMod's prebaked DLC from the clone plus our DLC at priority 350 with the `build/vendor/lekmod` overlay and the `src/lekmod` seam, drops our fork DLL over LekMod's (GUID kept), proxy / Tolk / cinematics / sounds. Mutually exclusive with the other states; `deploy.ps1` flips back to vanilla. `deploy-lekmod-sighted-multiplayer.ps1` is the partner variant.
- `resync-lekmod.ps1` — re-pins LekMod support in resumable phases (engine / mods / vendor / finish): rebase the `civvaccess` fork onto the new upstream commit, rebuild, gate on `tools/lekmod_dll_canary.py`, confirm the pulled DLC is pristine upstream, re-run vendor verify + `generate --engine lekmod`, then deploy and record the pin. No bake / modpack phase. See `docs/llm-docs/lekmod-support.md`.
- `package-lekmod-bundle.ps1` — the no-clone tester bundle (LekMod's DLC tree plus the `build/vendor/lekmod` overlay, the only non-committed inputs `deploy-lekmod` needs).
...
- `src/vp/` — the Community-Patch/Vox-Populi body of `CivVAccess_EngineData.lua`, swapped in under the shared include stem by every CP-DLL deploy (VP mod-overlay, VP modpack, and CP modpack alike — it feature-detects the balance mode, so one body serves both). The only source file that differs by engine (vanilla vs CP-DLL); everything else is shared and feature-detects at runtime. See `docs/llm-docs/cp-vp-support.md`.
