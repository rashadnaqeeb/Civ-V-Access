...
- `resync-vp.ps1` — re-pins VP support to a new upstream release in resumable phases (engine / mods / vendor / finish): rebase the fork, rebuild, gate on the DLL canary, mirror MODS to the clone, re-run vendor verify + generate, then deploy and record the new pin. See `docs/llm-docs/vox-populi.md` for the runbook.
