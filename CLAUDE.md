...
The build pipeline is split into three scripts at the repo root, with `build.ps1` as the orchestrator that calls them in order:

- `build-proxy.ps1` — compiles `lua51_Win32.dll` from `src/proxy/` to `dist/proxy/lua51_Win32.dll` (committed). Uses VS 2026's `cl.exe`.
- `build-engine.ps1` — compiles the engine DLL fork (`CvGameCore_Expansion2.dll`) from the Civ V Modding SDK source plus our overlay in `src/engine/`, output to `dist/engine/CvGameCore_Expansion2.dll` (committed). Uses VC9 (Visual C++ 2008 SP1) supplied by Windows SDK 7.0's `vc_stdx86` MSI. Engine builds take 1-2 minutes; only re-run when the source or overlay changes.
- `deploy.ps1` — copies the committed artifacts (`dist/proxy/lua51_Win32.dll`, `third_party/tolk/dist/x86/*` for the screen-reader runtime, `src/dlc/` payload, optionally `dist/engine/CvGameCore_Expansion2.dll`) into the game install. Engine DLL deploy is opt-in (`-DeployEngine`); on first install it backs up the vanilla `CvGameCore_Expansion2.dll` to `build/CvGameCore_Expansion2.vanilla.dll.bak`. The three deploy steps run independently of the build steps — contributors can pull and deploy without ever building.

Default `build.ps1` (no args) is the daily-iteration path: rebuilds the proxy, deploys proxy + DLC, leaves the engine alone. Use `build.ps1 -BuildEngine` to also rebuild and deploy the engine, `build.ps1 -DeployEngine` to deploy a previously-built engine DLL, and `build.ps1 -Uninstall` to remove everything (restoring vanilla engine DLL if a backup exists). Always use the scripts; never invoke `cl.exe` / `lua` / copy commands directly.

The engine fork toolchain (VC9 + Windows SDK 7.0) is set up by `build/sdk7-install/install.cmd`, which the maintainer runs once after downloading the SDK 7.0 ISO. Contributors who only want to deploy can skip that and use a prebuilt engine DLL.
...
- `src/dlc/` — fake-DLC payload. One BNW-only manifest (`CivVAccess_2.Civ5Pkg`, `<UISkin set="Expansion2">`), per-Context UI under `UI/InGame/` and `UI/FrontEnd/`, and cross-Context infra under `UI/Shared/` (listed under both `<Skin>` and `<GameplaySkin>` so either Context can `include` from it). Mod-authored locale strings live in per-Context files with distinct include stems because `include()` caches by bare stem on the shared lua_State. BNW is a hard requirement: if the active UISkin isn't Expansion2 the engine skips the manifest and the mod never loads — no fallback to base / G&K.
...
- **Engine bindings added by the fork only resolve on a fork deploy.** Any Lua method `src/engine/` adds (currently in `Lua/CvLuaUnit.cpp` and `Lua/CvLuaGame.cpp`, marked with `CIVVACCESS:` comments) is missing or stubbed in the vanilla DLL — `Unit:GeneratePath` is shipped as `luaL_error("NYI")`, the others aren't registered at all. Lua callers that depend on these must either be guarded by a feature check or the contributor must `deploy.ps1 -DeployEngine` (auto-implied by `build.ps1 -BuildEngine`). When adding a new binding, check the existing `CIVVACCESS:` markers for the registration pattern; the overlay is full-file replacement (no patch mechanism), so the file you modify must be a verbatim copy of the SDK source plus your additions.
