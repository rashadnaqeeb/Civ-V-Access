...
- A plain release (no upstream re-pin) leaves `vp_modpack`, `cp_modpack`, `vp_runtime`, and `lekmod_dlc` alone. They move only when `resync-vp.ps1` / `resync-lekmod.ps1` bumped them, which would already be reflected in `versions.json` (the re-sync scripts bump the fork, overlay, runtime, DLC, and modpack fields themselves through `tools/component-versions.ps1`). Do NOT hand-bump them - re-baking the modpacks needs manual merged-DB sessions, and a needless bump forces every mod player to redownload a ~600 MB package. Still verify the coupling below against `git diff <prev-tag> -- dist/engine-vp dist/engine-lekmod versions.json`: a re-pin whose modpack phase was never run leaves `vp_modpack` / `cp_modpack` unbumped behind a moved fork. One exception to hands-off: `lekmod_dlc` also carries the map-generation progress hook that `tools/dlc-assembly.ps1` splices into Lekmap Pangaea v6.2 (`Add-CivVAccessLekmapProgressHook`), so if that function changed since the prev tag and `lekmod_dlc` has not otherwise moved, bump it, or the release re-uploads the prior zip with the old hook.
