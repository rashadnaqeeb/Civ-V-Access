...
def dump_gameplay(db_path, out_path):
    ...
    for t in tables:
        ...
        parts.append(f"\t</{t}>")
    # Symmetric with the CustomModOptions SQUADS guard: forcing SQUADS=1 without
    # the squad interface modes crashes CP/VP's in-game UI. If the cache had no
    # InterfaceModes table at all, the injection above never ran -- refuse rather
    # than bake that crash.
    if FORCE_INTERFACE_MODES and not interface_modes_seen:
        raise SystemExit(
            "No InterfaceModes table in the cache, so the squad interface "
            "modes were not injected while SQUADS is forced on -- the baked "
            "pack would crash CP/VP's UI on load. The cache is not a valid "
            "CP/VP merge; replay a clean session.")
