...
-- live cursor, behind the scannerDirectionScope setting. The arc apex is
-- re-anchored to the cursor on every navigation-key rebuild (cycleItem
-- here), so an item that falls behind the roaming cursor is pruned on the
-- next nav read; read-only probes (End) keep the focused item by skipping
-- the arc filter.

local function dirSetup()
    setup()
    civvaccess_shared.scannerDirectionScope = true
...
function M.test_set_direction_is_inert_while_the_setting_is_off()
    dirSetup()
    -- Same fixture as above with the opt-in toggle off: the keypress has
    -- nothing to say and the list keeps both cities, so a player who never
    -- enabled the feature can't end up with a silently filtered scanner.
    civvaccess_shared.scannerDirectionScope = false
    T.installMap({ mkPlot(-5, 0, 0), mkPlot(5, 0, 1) })
    _entries = {
        mkEntry("cities", "my", "West", 0, { key = "stub:west" }),
        mkEntry("cities", "my", "East", 1, { key = "stub:east" }),
    }
    T.eq(ScannerNav.setDirection("W"), nil, "a gated press speaks nothing")
    T.eq(civvaccess_shared.scannerDirection, nil, "no scope is recorded")
    ScannerNav.cycleItem(1)
    T.eq(#citiesAllItems(ScannerNav._snapshot()), 2, "both cities stay in the unscoped list")
end
