...
-- ===== bearingInArc =====
-- Membership predicate for the directional scanner's 90-degree arcs.
-- Center angles follow the atan2 convention: E = 0, N = pi/2, W = pi,
-- S = 3pi/2; half-width pi/4 tiles the circle into four gap-free arcs.
local PI = math.pi
local ARC_E, ARC_N, ARC_W, ARC_S = 0, PI / 2, PI, 3 * PI / 2
local HALF = PI / 4

local function arcSetup()
    setupSteps()
    Map.IsWrapX = function()
        return false
    end
...
function M.test_bearingInArc_due_west_passes_west_only()
    arcSetup()
    -- (0,0) -> (-3,0): pixel bearing pi, dead center of the W arc.
    T.truthy(HexGeom.bearingInArc(0, 0, -3, 0, ARC_W, HALF), "due west must pass the W arc")
    T.falsy(HexGeom.bearingInArc(0, 0, -3, 0, ARC_E, HALF), "due west must fail the E arc")
    T.falsy(HexGeom.bearingInArc(0, 0, -3, 0, ARC_N, HALF), "due west must fail the N arc")
    T.falsy(HexGeom.bearingInArc(0, 0, -3, 0, ARC_S, HALF), "due west must fail the S arc")
end

function M.test_bearingInArc_due_east_passes_east_only()
    arcSetup()
    T.truthy(HexGeom.bearingInArc(0, 0, 3, 0, ARC_E, HALF), "due east must pass the E arc")
    T.falsy(HexGeom.bearingInArc(0, 0, 3, 0, ARC_W, HALF), "due east must fail the W arc")
end

function M.test_bearingInArc_due_north_passes_north()
    arcSetup()
    -- (0,0) -> (0,2): a 1ne+1nw zigzag whose pixel bearing is pi/2 (due N).
    T.truthy(HexGeom.bearingInArc(0, 0, 0, 2, ARC_N, HALF), "due north must pass the N arc")
    T.falsy(HexGeom.bearingInArc(0, 0, 0, 2, ARC_S, HALF), "due north must fail the S arc")
end

function M.test_bearingInArc_due_south_passes_south()
    arcSetup()
    T.truthy(HexGeom.bearingInArc(0, 0, 0, -2, ARC_S, HALF), "due south must pass the S arc")
    T.falsy(HexGeom.bearingInArc(0, 0, 0, -2, ARC_N, HALF), "due south must fail the N arc")
end

function M.test_bearingInArc_ne_hex_step_lands_north_of_boundary()
    arcSetup()
    -- A single NE hex step sits at 60 degrees on the unit circle, past the
    -- 45-degree E/N boundary, so it belongs to N and not E.
    T.truthy(HexGeom.bearingInArc(0, 0, 0, 1, ARC_N, HALF), "60-degree NE step is on the N side of the boundary")
    T.falsy(HexGeom.bearingInArc(0, 0, 0, 1, ARC_E, HALF), "60-degree NE step must not fall in the E arc")
end

function M.test_bearingInArc_zero_delta_always_true()
    arcSetup()
    -- Cursor sitting on the target: in-arc for every direction so a jump
    -- onto an item never makes it vanish.
    T.truthy(HexGeom.bearingInArc(5, 5, 5, 5, ARC_E, HALF))
    T.truthy(HexGeom.bearingInArc(5, 5, 5, 5, ARC_N, HALF))
    T.truthy(HexGeom.bearingInArc(5, 5, 5, 5, ARC_W, HALF))
    T.truthy(HexGeom.bearingInArc(5, 5, 5, 5, ARC_S, HALF))
end

function M.test_bearingInArc_wrapX_one_west_reads_west_not_east()
    -- Width-80 wrap. (0,10) -> (79,10) is 79 east by naive subtraction but
    -- one tile west across the seam; the arc must read it as W, not E.
    wrapSetup(80, 40, true, false)
    T.truthy(HexGeom.bearingInArc(0, 10, 79, 10, ARC_W, HALF), "wrapped one-west target must pass the W arc")
    T.falsy(HexGeom.bearingInArc(0, 10, 79, 10, ARC_E, HALF), "wrapped one-west target must fail the E arc")
end
